...
import '../../Components/CardPost/card.css'


const CardsOfPerfils = (props) => {

  let userPosts = props.usersDt.filter(item=> item.uid===props.slug)
  ...
  return (
  <div className='card-display w-[90%] min-w-[300px] max-w-[620px] h-auto flex flex-col gap-2 items-start  border-[#cdcdcd7b] border-[3px] p-2 rounded-3xl'>
    <div className=' flex items-center gap-5 self-start cursor-pointer' onClick={()=>navigate(`/perfils/${userdata.uid}`)}>
      <img src={userdata.photoURL} alt={userdata.displayName} className='w-16 h-16 rounded-full border-black border-[1px]' />
    ...
    <p className='font-medium text-[1.3rem] '>Fecha del post {formattedDate|| '1 de julio 2023'}</p>
    ...
    <p className='w-[90%] min-w-[280px] max-w-[600px] h-auto '>{props.offer}</p>
